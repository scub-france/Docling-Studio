# Design: Move store connection (URI/user/password) into the Store entity
...
- **Issue:** #279
- **Title on issue:** [FEATURE] Move store connection (URI/user/password) into the Store entity
- **Author:** Pier-Jean Malandrino
- **Date:** 2026-05-18
- **Status:** Accepted

> **Note on design depth:** §3–§11 are intentionally light — the design was
> negotiated in conversation prior to scaffolding (scope adjustment recorded
> as a comment on #279, dated 2026-05-18). Implementation steps and risks
> live in the PR description; this doc keeps the canonical issue link and
> the goals checklist that drives acceptance.
...
Today the `Store` entity in SQLite captures only **application-level** config (name, slug, kind, embedder, `vector_index_name`, `database`). The **connection** to the backing store (Neo4j URI/user/password, OpenSearch URL) lives in env vars (`NEO4J_URI`, `NEO4J_USER`, `NEO4J_PASSWORD`, `OPENSEARCH_URL`) and is materialised into a **single** driver per kind at backend boot (`main.py::_init_neo4j`).

This creates a half-baked abstraction the user can directly feel in the UI: the "Edit store" form shows a Neo4j store but doesn't ask for URI / user / password. Where do credentials come from?

Symptoms / consequences:

- The store form lies about what it configures — it pretends to be a self-contained "Store" but actually shares a process-global connection.
- All stores of `kind = "Neo4j"` write to the **same** Neo4j instance, regardless of what the user creates in the UI. Same for OpenSearch.
- True multi-cluster / multi-instance (e.g. `Neo4j Local` + `Neo4j Staging`) is impossible without a backend redeploy + env-var swap.
- Connection identity is invisible in the UI: nothing tells the user `Neo4j Local` and `Neo4j Prod` actually hit the same physical instance.

The cause is historical: the `stores` table was introduced by #205 to track push audit, then grew into a CRUD UI for multi-store dispatch (#225) — but the "what is a Store, really?" question was never decided.
...
- [ ] Create a new Neo4j store via the UI with URI/user/password → push works against that exact instance.
- [ ] Create two Neo4j stores pointing to two different Neo4j containers (`bolt://localhost:7687` and `bolt://localhost:7688`) → pushing to each writes to the right one, verified by Cypher count on both.
- [ ] Same scenario for OpenSearch (two different cluster URLs).
- [ ] Backend restart → password persists (encrypted), driver pool re-hydrates lazily on first push.
- [ ] `GET /api/stores` and `GET /api/stores/{id}` never include `connectionPassword` in the response.
- [ ] `PATCH /api/stores/{id}` with empty `connectionPassword` keeps the existing value; with a non-empty value, replaces it.
- [ ] `POST /api/stores/{id}/test-connection` returns `{ ok: true }` on a live store, `{ ok: false, error: "…" }` on a wrong password.
- [ ] Form "Test connection" button hits the endpoint and shows green/red feedback before save.
- [ ] Boot with stores containing encrypted secrets but no `STORE_SECRET_KEY` → backend exits with a clear error.
- [ ] Backend tests cover: encryption roundtrip, driver pool keying, test-connection endpoint, password masking on serialisation.
- [ ] Frontend tests cover: form validation per kind, password "unchanged" placeholder behaviour, test-connection action.
- [ ] Validation pipeline green (`ruff` + `pytest` + frontend lint/type/test).
...
- **Issue:** https://github.com/scub-france/docling-Studio/issues/279
- **Related PRs / commits:**
